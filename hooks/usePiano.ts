...
function normalizeNoteName(noteName: string): string {
  ...
  const mapped = FLAT_TO_SHARP[`${base}${accidental}`];
  return mapped ? `${mapped}${octave}` : noteName;
}

function resolveNoteFrequency(noteName: string): number | undefined {
  const normalized = normalizeNoteName(noteName);
  return NOTES[normalized as keyof typeof NOTES];
}
...
export function usePiano() {
  ...
  const performStopNote = (noteName: string, keyElement?: HTMLElement) => {
    ...
    if (sustainPedalRef.current) {
      ...
      setTimeout(() => {
        ...
        releasedNotesInSustainRef.current =
          releasedNotesInSustainRef.current.filter(
            (n) => n.noteName !== noteName,
          );
      }, 3200);
      activeOscillatorsRef.current.delete(noteName);
      return;
    ...
    if (
      ...
      recordIndex >= 0 &&
      recordedNotesRef.current[recordIndex]
    ) {
      recordedNotesRef.current[recordIndex].duration =
        audioContext.currentTime -
        recordStartTimeRef.current -
        recordedNotesRef.current[recordIndex].startTime;
    }
  };
  ...
  const playNote = useCallback(
    (noteName: string, frequency: number, keyElement?: HTMLElement) => {
      ...
      const clickBufferSize = Math.ceil(
        audioContext.sampleRate * clickDuration,
      );
  ...
  const releaseSustainedNotes = useCallback(() => {
    ...
    releasedNotesInSustainRef.current.forEach(
      ({ noteName, oscillators, masterGain: noteGain, recordIndex }) => {
  ...
  const playRecording = useCallback(
    (
      ...
      const totalDuration =
        Math.max(...notes.map((n) => n.startTime + n.duration)) + 0.2;
  ...
  const downloadRecording = useCallback(async () => {
    ...
    const totalDuration =
      Math.max(...notes.map((n) => n.startTime + n.duration + 3)) + 1;
    ...
    notes.forEach(({ frequency, startTime, duration }) => {
      ...
      harmonicConfig.forEach(([n, amp, decayTime]) => {
        ...
        gain.gain.exponentialRampToValueAtTime(
          Math.min(amp * 0.4, gain.gain.value),
          Math.min(releaseAt, startTime + decayTime * 0.8),
        );
  ...
  const playSongSequence = useCallback(
    (
      ...
      const playNextNote = (index: number) => {
        ...
        const noteNames = Array.isArray(note) ? note : [note];
        const explicitFrequencies = Array.isArray(freq) ? freq : undefined;

        if (noteNames.length === 1 && noteNames[0] === "REST") {
          const restTimer = setTimeout(() => {
            playNextNote(index + 1);
          }, duration * 1000);
          timerId.push(restTimer);
          return;
        }

        const playableNotes = noteNames
          .map((noteName, noteIndex) => {
            ...
            const frequency =
              explicitFrequencies?.[noteIndex] ||
              (!Array.isArray(freq) && noteNames.length === 1
                ? freq
                : undefined) ||
              resolveNoteFrequency(noteName);
          ...
          .filter((entry): entry is { noteName: string; frequency: number } =>
            Boolean(entry),
          );

        if (playableNotes.length === 0) {
          // 未识别的音符仍然保留原本时值，避免整首曲子的节奏被压缩。
          const skipTimer = setTimeout(() => {
            playNextNote(index + 1);
          }, duration * 1000);
          timerId.push(skipTimer);
          return;
        }

        playableNotes.forEach(({ noteName, frequency }) => {
          onNoteOn?.(noteName);
          playNote(noteName, frequency);
        });

        const stopTimer = setTimeout(() => {
          playableNotes.forEach(({ noteName }) => {
            stopNote(noteName);
            onNoteOff?.(noteName);
          });
          setTimeout(() => {
